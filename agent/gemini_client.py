...
class GeminiLLMClient:
    ...
    def generate(self, prompt: str, **kwargs: Any) -> str:
        ...
        try:
            # Attempt to use the full SDK with GenerateContentConfig.
            # When a mock/injected client is used (no real SDK), fall through
            # to the lightweight call path below.
            try:
                from google.genai import types as _genai_types  # type: ignore[import]  # noqa: F401
                ...
                # SDK not installed – call the client without a typed config
                # (covers injected mocks in offline tests).
                response = self.client.models.generate_content(
                    model=model,
                    contents=prompt,
                )
